...
INSTALLED_APPS = [
    ...
    'cambios',
    'rest_framework',
    'rest_framework.authtoken',  # legacy: ya no se usa para auth (JWT lo reemplaza) pero la tabla queda en BD
...
# Access corto + refresh largo con rotation y blacklist tras rotacion.
...
# Si no hay credenciales SMTP, usar dummy backend (no hace red). Esto evita
# que send_mail() se cuelgue intentando conectar a Gmail por >30s y mate al
# worker de gunicorn con SIGKILL (SystemExit que no se atrapa con except).
if EMAIL_HOST_USER and EMAIL_HOST_PASSWORD:
    ...
    EMAIL_USE_TLS = True
    EMAIL_USE_SSL = False
    EMAIL_TIMEOUT = 5  # timeout corto para no agotar el worker
else:
    EMAIL_BACKEND = "django.core.mail.backends.dummy.EmailBackend"

DEFAULT_FROM_EMAIL = EMAIL_HOST_USER or "no-reply@sgmsc.local"
